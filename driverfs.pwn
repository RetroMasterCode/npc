...
#define NPC_NAMES           	"NPCDriver_%d" // %d будет заменен ID водителя (не NPC ID!)

#define DEBUG_BUBBLE			(false) // Позволяет NPC показывать информацию через пузыри чата
#define DEBUG_PRINTS			(false) // Отображает время расчета и предупреждения
#define INFO_PRINTS				(true) // Отображает информацию о водителях каждые X секунд
#define INFO_DELAY				(300) // Секунды
#define MAP_ZONES				(false) // Создает зоны банд для каждого водителя в качестве замены маркера карты (все NPC всегда видны в ESC-> Карта)
#define SEND_DEATH_MESSAGE		(false) // Отправляет сообщение о смерти для убитого NPC в чате (измените OnPlayerDeath в соответствии с вашим сервером)

#define DRIVER_AMOUNT			(550)  	// ОБЩЕЕ КОЛИЧЕСТВО NPC — различные типы водителей являются частью общего количества водителей (300/20/20 = 300 NPCS, из которых 20 такси, 20 копов и 260 нормичей)
#define DRIVER_TAXIS			(50)
#define DRIVER_COPS             (35)

#define MAX_NODE_DIST			(18.0)
#define MIN_NODE_DIST			(2.5) // Небольшие изменения здесь обычно имеют большое значение. Не опускайтесь ниже 1,5.
#define SIDE_DIST				(2.02) // Расстояние до центра дороги, 2 м обычно оптимальное (к сожалению, фактические данные о размерах дорог не включены в плагин GPS)
#define SMOOTH_W_DATA           (0.6) // Значения сглаживания, DATA - вес данных, SMOOTH - вес сглаживания, должен быть между 0,1 и 1,0.
#define SMOOTH_W_SMOOTH         (0.2)
#define SMOOTH_AMOUNT           (20) // Количество проходов сглаживания — раньше было динамическим, но из-за этого его было сложно ограничить — среднее сглаживание — 15 (данные 0,6, вес 0,2)

#define MIN_SPEED				(0.7)
#define MAX_SPEED				(2.2)
#define DUTY_SPEED_BOOST        (1.125) // Повышение скорости при исполнении служебных обязанностей (полицейские, такси)
#define STEER_ANGLE             (5.0)

#define JAM_DIST               	(15.0) // Расстояние между двумя водителями, чтобы они замедлились
#define JAM_ANGLE               (25) // INT! Максимальное угловое расстояние между двумя водителями, чтобы они замедлились

#define MAX_PATH_LEN    		(1350)

#define MAX_TAXI_DIST           (3000.0) // Максимальное расстояние до такси, чтобы ответить
#define TAXI_RANGE				(35.0) // Максимальный диапазон до ближайших узлов (при вызове)
#define TAXI_COOLDOWN			(60) // секунды
#define TAXI_TIMEOUT			(40) // секунды

#define ROUTE_MIN_DIST			(900.0)
#define ROUTE_MAX_DIST          (2400.0)

#define DRIVERS_ROUTE_ID		(10000) // Начальный routeid для расчета пути - изменить, если возникают конфликты
#define DIALOG_ID				(10000) // Начальный dialogid для диалогов - изменить при возникновении конфликтов
...
#define MAX_PATH_NODES			(800) // Максимальное количество начальных и конечных узлов
...
#define ZONES_NUM				(90) // Это просто для определения расстояний NPC друг от друга с помощью целых чисел, меньшее значение означает большие зоны -> больше NPC для проверки.

#define DID_TAXI				(DIALOG_ID + 0)

#pragma dynamic					(50000) // Должно быть выше для более длинных путей/большего количества NPC (для 900 — вы можете уменьшить или удалить его, если используете меньше 400)!

// ----------------------------------------------------------------------------- ОПРЕДЕЛИТЕ, ГДЕ ПОЯВЛЯЮТСЯ/ПЕРЕХОДЯТ NPC. Если вы сузите круг до небольшой области, уменьшите количество NPC пропорционально!

#define MAP_ENABLE_LS           (true) // Обратите внимание, что если вы включите (например) только LV и SF, драйверы, скорее всего, будут переключаться с LV на SF и наоборот.
...
new Float:LSCoords[4][4] = // maxx, maxy, minx, miny — создано с помощью редактора GTA Zone от zeppelin — довольно грубо, но отлично работает для узлов
{
...
enum E_DRIVERS
{
	bool:nUsed,
	bool:nOnDuty,
	bool:nActive, // Активно означает, что игрок находится близко (-> выполняет все расчеты, в противном случае пропускает некоторые узлы и не обрабатывает столкновение/вращение)
	nNPCID,
	nType,
	nState,
	nCurNode,
...
new PlayerEnterDriver[MAX_PLAYERS] = {-1, ...};

new Float:VehicleZOffsets[] = // Содержит обычные 4-колесные транспортные средства, в том числе квадроциклы, полицейские машины и полицейские ранчеры, поскольку расчеты угла также включают некоторые велосипеды.
{
...
new DriverSkins[] = // Идентификаторы скинов для граждан, не отсортированные - нет конкретных/сюжетных скинов
{
...
new CopSkins[] = // Идентификаторы скинов для копов
{
	280, 281, 282, 283, 288, 306, 307, 310, 311
};

new TaxiSkins[] = // Идентификаторы скинов для таксистов - выбраны случайно
{
	188, 20, 36, 262, 7, 56
};

new MapNode:PathNodes[MAX_PATH_NODES], PathNodesNum = 0; // Начальные и конечные узлы для путей — генерируются при инициализации — всегда используйте новейший GPS.dat, чтобы иметь достаточно узлов с 1 соединением и хорошо разбросанных NPC!
// Я буду собирать такие узлы, пока у нас не будет около 1200 или даже больше.
new IgnoredPathNodes[] = // Узлы, которые следует игнорировать для начальных/конечных узлов - в основном слишком много в одном месте, если не указано иное - НЕ будут игнорироваться при обычном вождении.
{
	// Parking LS (too many)
...
new rescueid = 0; // Текущий идентификатор для проверки в RescueTimer (проверяет только несколько записей (20) при каждом вызове, чтобы предотвратить длинные циклы)
...
new NumRouteCalcs = 0, ExitPlayerID = -1; // Важно для плавной разгрузки FS
...
Drivers_Init()
{
	...
	CA_Init(); // Вы должны раскомментировать это, если вы не инициализируете ColAndreas до загрузки этой FS!
	...
	{
	    if(strlen(gDestinationDialogSTR) >= sizeof(gDestinationDialogSTR) - strlen(gDestinationList[i][destName]) - 10) break; // В случае, если gDestinationDialogSTR слишком мал, остановитесь на последнем подходящем телепорте, чтобы предотвратить отключение.
	...
	// ---------------- СОЗДАТЬ НАЧАЛЬНЫЙ И КОНЕЧНЫЙ УЗЛЫ
	...
	// ---------------- ПОДКЛЮЧЕНИЕ NPCS и прочее

	for(new i = 0; i <= 211; i ++) // Создайте список транспортных средств для использования
	{
...
public FCNPC_OnStreamIn(npcid, forplayerid)
{
	...
	if(PlayerEnterDriver[forplayerid] != -1 && PlayerEnterDriver[forplayerid] == driverid)
	{
		PutPlayerInVehicle(forplayerid, Drivers[driverid][nVehicle], 1);
		PlayerEnterDriver[forplayerid] = -1;
	}
...
{
	if(!Initialized || !IsPlayerAdmin(playerid)) return 0;

	new id = -1;

	if(!isnull(params) && strlen(params) < 8)
	{
		if(params[0] == 'r') id = random(DRIVER_AMOUNT);
		else
		{
			id = strval(params);

			if(id < 0 || id >= DRIVER_AMOUNT) id = -1;
		}
	}

	if(id == -1) return SendClientMessage(playerid, -1, "Invalid Driver ID");

	new Float:x, Float:y, Float:z;
	FCNPC_GetPosition(Drivers[id][nNPCID], x, y, z);

	if(FCNPC_IsStreamedIn(Drivers[id][nNPCID], playerid)) PutPlayerInVehicle(playerid, Drivers[id][nVehicle], 1);
	else
	{
		SetPlayerPos(playerid, x, y, z);
		PlayerEnterDriver[playerid] = id;
	}

	return 1;
}

COMMAND:dfs_cmds(playerid, params[])
{
...
COMMAND:dfs_exit(playerid, params[])
{
	if(!Initialized || !IsPlayerAdmin(playerid)) return 0;
	...
	{
		new text[128];
		format(text, sizeof(text), "There are %d Path Calculations left. The Script will unload once they are completed.", NumRouteCalcs);
...
COMMAND:dfs_info(playerid, params[])
{
	if(!Initialized || !IsPlayerAdmin(playerid)) return 0;
...
public OnPathCalculated(Path:pathid, routeid)
{
	...
	if(routeid >= DRIVERS_ROUTE_ID && routeid < DRIVERS_ROUTE_ID + DRIVER_AMOUNT) // the routeid given comes from this script
	{
		...
		/*
		Объяснение цикла (ниже)

		i - индекс для записи (для массива newpath)
		arrayid — индекс для чтения (для node_id_array)

		Целевой узел будет оставаться до тех пор, пока расстояние слишком велико.
		Целевой узел будет пропущен, если расстояние слишком мало.
...
public FCNPC_OnReachDestination(npcid)
{
    ...
    if(driverid != -1)
	{
		...
		if(!FCNPC_IsStreamedInForAnyone(npcid))
		{
		    ...
		    FCNPC_GoTo(npcid, DriverPath[driverid][cnode][0], DriverPath[driverid][cnode][1], DriverPath[driverid][cnode][2], FCNPC_MOVE_TYPE_DRIVE, MAX_SPEED*0.8, .pathfinding = FCNPC_MOVE_PATHFINDING_NONE, .radius = 0.0, .set_angle = true, .min_distance = 0.0, .stop_delay = 0);
		...
		#if DEBUG_BUBBLE == true
		new bool:complex, Float:z_angle;
		#endif

		if(cnode > 0 && cnode < DriverPathLen[driverid]-1)
		{
			new Float:z_angle1, Float:z_angle2, Float:waste, Float:surface_rx, Float:surface_ry;

			GetRZFromVectorXY(DriverPath[driverid][cnode][0] - DriverPath[driverid][cnode - 1][0], DriverPath[driverid][cnode][1] - DriverPath[driverid][cnode - 1][1], z_angle1);
			GetRZFromVectorXY(DriverPath[driverid][cnode + 1][0] - DriverPath[driverid][cnode][0], DriverPath[driverid][cnode + 1][1] - DriverPath[driverid][cnode][1], z_angle2);

			#if DEBUG_BUBBLE == true
			z_angle = z_angle1;
			#endif

			new Float:angle_dif = floatangledistdir(z_angle1, z_angle2);

		    FCNPC_SetKeys(npcid, 0, (angle_dif <= -STEER_ANGLE ? 128 : (angle_dif >= STEER_ANGLE ? -128 : 0)), 0);

		    CA_RayCastLineAngle(X, Y, Z + 0.75, X, Y, Z - 3.0, waste, waste, waste, surface_rx, surface_ry, waste);

		    if(Drivers[driverid][nVehicleIsBike])
		    {
		    	angle_dif = -angle_dif / 15.0;

		    	if(angle_dif < -1.0) angle_dif = -1.0;
		    	else if(angle_dif > 1.0) angle_dif = 1.0;

		    	angle_dif = (angle_dif + Drivers[driverid][nVehicleLastLean] + Drivers[driverid][nVehicleLastLean]) / 3.0;

		    	Drivers[driverid][nVehicleLastLean] = angle_dif;

		    	GetRotForSurface(Qw, Qx, Qy, Qz,  surface_rx, surface_ry,  0.0, angle_dif * 30.0, z_angle1);
		    }
		    else GetRotForSurface(Qw, Qx, Qy, Qz,  surface_rx, surface_ry,  0.0, 0.0, z_angle1);
		}
		else
		{
		    FCNPC_SetKeys(npcid, 0, 0, 0);
		    ...
		    if(cnode > 0)
		    {
			    new Float:z_angle1, Float:rx;

			    GetRZFromVectorXY(DriverPath[driverid][cnode][0] - DriverPath[driverid][cnode - 1][0], DriverPath[driverid][cnode][1] - DriverPath[driverid][cnode - 1][1], z_angle1);
			    GetRXFromVectorZ(DriverPath[driverid][cnode][2] - DriverPath[driverid][cnode - 1][2], rx);

		    	GetRotForSurface(Qw, Qx, Qy, Qz,  0.0, 0.0,  rx, 0.0, z_angle1);
		    }
		    else GetRotForSurface(Qw, Qx, Qy, Qz,  0.0, 0.0,  0.0, 0.0, FCNPC_GetAngle(npcid));
		}

		FCNPC_GoTo(npcid, DriverPath[driverid][cnode][0], DriverPath[driverid][cnode][1], DriverPath[driverid][cnode][2], FCNPC_MOVE_TYPE_DRIVE, Drivers[driverid][nSpeed], .pathfinding = FCNPC_MOVE_PATHFINDING_NONE, .radius = 0.0, .set_angle = true, .min_distance = 0.0, .stop_delay = 0);
...
Float:floatangledistdir(Float:firstAngle, Float:secondAngle) // Ranging from -180 to 180 (directional)
{
...
GetRotForSurface(&Float:qw, &Float:qx, &Float:qy, &Float:qz, Float:surface_rx, Float:surface_ry, Float:offset_rx = 0.0, Float:offset_ry = 0.0, Float:offset_rz = 0.0) // By NaS
{
	...
	GetRotationMatrixFromEuler(matrix, offset_rx, offset_ry, offset_rz, rot_mode);

	if(surface_rx != 0.0 || surface_ry != 0.0) RotateMatrixWithEuler(matrix, surface_rx, surface_ry, 0.0, rot_mode);

	new Float:euler_rx, Float:euler_ry, Float:euler_rz;

	GetEulerFromMatrix(matrix, euler_rx, euler_ry, euler_rz);
	GetQuatFromEuler(euler_rx, euler_ry, euler_rz, qw, qx, qy, qz, rot_mode);
...
GetRZFromVectorXY(Float:vx, Float:vy, &Float:a)
{
	...
	new Float:len = VectorSize(vx, vy, 0.0);
